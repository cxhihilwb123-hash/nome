        ...
        VStack {
            List {
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    Text("Share public contact address on social media.")
                    Text("You won't lose your contacts if you later delete your address.")

                    (Text(Image(systemName: "link.badge.plus")).foregroundColor(.secondary) + textSpace + Text("Share one-time friend link").font(.title2).bold())
                        .padding(.top)
                    ...
                    if !showCreateAddressButton {
                        (Text(Image(systemName: "shield")).foregroundColor(.secondary) + textSpace + Text("Connection security").font(.title2).bold())
                            .padding(.top)
                        Text("Public contact addresses and one-time links are safe to share via any messenger.")
    ...
    private func createOneTimeLinkButton() -> some View {
        ZStack {
            Button {
                ...
                Text("Create one-time friend link")
                    .font(.callout)
            }

            NavigationLink(isActive: $createOneTimeLinkActive) {
                NewChatView(selection: .invite)
                    .navigationTitle("Add or join")
                    .navigationBarTitleDisplayMode(.large)
