...
class NomeDatabaseRootScreenshotTest {
  @Test
  fun captureRendererMatrix() {
    ...
    for (captureCase in cases) {
      ...
      val scenario: ActivityScenario<NomeDatabaseRootEvidenceActivity> =
        ActivityScenario.launch(
          NomeDatabaseRootEvidenceActivity.intent(
            targetContext,
            captureCase.spec,
          ),
        )
  ...
  private fun captureCases(): List<CaptureCase> = buildList {
    for (state in NomeDatabaseRootEvidenceState.entries) {
  ...
  private data class CaptureCase(
    val spec: NomeDatabaseRootEvidenceSpec,
  ) {
    val fileName: String
      get() = buildString {
        append("native-p01-")
  ...
  private companion object {
    ...
    const val EVIDENCE_DIRECTORY = "nome-batch1a-p01-evidence"
    const val EXPECTED_CAPTURE_COUNT = 112
