...
testOperators =
  testChatCfgOpts testCfg opts' aliceProfile $
    \alice -> do
      ...
      alice ##> "/_operators"
      alice <##. "1 (nome). Nome, domains: nome.im, servers: enabled, conditions: accepted"
      -- update operators
      alice ##> "/operators 1:on:smp=proxy:xftp=off"
      alice <##. "1 (nome). Nome, domains: nome.im, servers: SMP enabled proxy, XFTP disabled (servers known), conditions: accepted"
  where
...
testMultipleUserAddresses =
  testChat3 aliceProfile bobProfile cathProfile $
    \alice bob cath -> do
      ...
      bob ##> ("/c " <> cLinkAlisa)
      alice <#? bob
      alice #$> ("/_get chats 2 pcc=on", chats, [("@bob", "Audio/video calls: enabled"), ("*", "")])
      alice ##> "/ac bob"
      alice <## "bob (Bob): accepting contact request, you can send messages to contact"
      concurrently_
...
testUsersDifferentCIExpirationTTL ps = do
  withNewTestChat ps "bob" bobProfile $ \bob -> do
    withNewTestChatCfg ps cfg "alice" aliceProfile $ \alice -> do
      ...
      alice #$> ("/_get chat @4 count=100", chat, [(1,"chat banner")])
  where
...
testDisableCIExpirationOnlyForOneUser ps = do
  withNewTestChat ps "bob" bobProfile $ \bob -> do
    ...
    withTestChatCfg ps cfg "alice" $ \alice -> do
      ...
      alice #$> ("/_get chat @4 count=100", chat, [(1,"chat banner"), (1, "alisa 3"), (0, "alisa 4")])
...
testUsersTimedMessages ps' = do
  withNewTestChat ps "bob" bobProfile $ \bob -> do
    withNewTestChat ps "alice" aliceProfile $ \alice -> do
      ...
      alice ##> "/user alisa"
      showActiveUser alice "alisa"
      alice #$> ("/_get chat @4 count=100", chat, [(1,"chat banner"), (1, "alisa 1"), (0, "alisa 2")])
