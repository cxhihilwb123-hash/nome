...
make_repo() {
  name=$1
  branch=${2:-codex/nome-v656-consolidated-android}
  repo="$work_dir/$name"
  mkdir -p "$repo"
...
(
  cd "$clean_repo"
  "$gate" --platform android --allowed-branch codex/nome-v656-consolidated-android --config release.config
) > "$work_dir/clean.log"
...
expect_fail tracked-dirty sh -c "cd '$dirty_repo' && '$gate' --platform android --allowed-branch codex/nome-v656-consolidated-android --config release.config"

staged_repo=$(make_repo staged-dirty)
printf 'changed\n' >> "$staged_repo/source.txt"
git -C "$staged_repo" add source.txt
expect_fail staged-dirty sh -c "cd '$staged_repo' && '$gate' --platform android --allowed-branch codex/nome-v656-consolidated-android --config release.config"

untracked_repo=$(make_repo untracked-source)
printf 'untracked source\n' > "$untracked_repo/NewFeature.kt"
expect_fail untracked-source sh -c "cd '$untracked_repo' && '$gate' --platform android --allowed-branch codex/nome-v656-consolidated-android --config release.config"

wrong_branch_repo=$(make_repo wrong-branch main)
expect_fail wrong-branch sh -c "cd '$wrong_branch_repo' && '$gate' --platform android --allowed-branch codex/nome-v656-consolidated-android --config release.config"

artifact_repo=$(make_repo artifact)
printf 'artifact bytes\n' > "$work_dir/Nome-test.artifact"
(
  cd "$artifact_repo"
  "$gate" --platform android --allowed-branch codex/nome-v656-consolidated-android \
